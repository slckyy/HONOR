...
async def _emit_fact_event(conn, *, event_name: str, entity_type: str, entity_id: str, correlation_id: str, payload: dict[str, Any], idempotency_key: str):
    await conn.execute("""
        INSERT INTO events(id,event_name,event_version,occurred_at,actor_type,source_service,correlation_id,entity_type,entity_id,idempotency_key,payload)
...
async def import_campaign(request: Request, owner: str = Depends(require_owner), idempotency_key: str | None = Header(default=None, alias="Idempotency-Key")):
    ...
    async with owner_transaction(owner) as conn:
        ...
        if body.get("terms_upload_id"):
            terms_upload = await conn.fetchrow("SELECT object_key,expected_sha256,verified_sha256,state FROM uploads WHERE id=$1::uuid AND owner_user_id=$2::uuid", body["terms_upload_id"], owner)
        ...
        terms_text = body.get("terms_text") or f"OWNER_IMPORT:{body['provider']}:{body.get('external_campaign_id') or ''}"; terms_bytes = terms_text.encode(); digest = hashlib.sha256(terms_bytes).hexdigest()
        snapshot_key = terms_upload["object_key"] if terms_upload else f"evidence/campaigns/{campaign_id}/{snapshot_id}.terms"
        snapshot_sha = terms_upload["verified_sha256"] if terms_upload else digest
        if not terms_upload:
            storage().put(snapshot_key, terms_bytes, "text/plain")
        # OWNER_URL supplies a locator only; no page was fetched or captured.
        # MANUAL_ENTRY is the frozen provenance classification for that fact.
        capture_method = "OWNER_UPLOAD" if terms_upload else "MANUAL_ENTRY"
...
async def import_source(request: Request, owner: str = Depends(require_owner), idempotency_key: str | None = Header(default=None, alias="Idempotency-Key")):
    ...
    async with owner_transaction(owner) as conn:
        ...
        if replay is not None: return replay
        source_upload = None
        if body["method"] == "MANUAL_UPLOAD":
            source_upload = await conn.fetchrow("SELECT object_key,verified_sha256,state FROM uploads WHERE id=$1::uuid AND owner_user_id=$2::uuid", body["upload_id"], owner)
            if source_upload is None or str(source_upload["state"]) != "VERIFIED" or not source_upload["verified_sha256"]:
                raise HonorError(409, "CONFLICT", "Source upload must be owned by the owner and VERIFIED.")
            ingest = "READY"
        await conn.execute("INSERT INTO sources(id,origin_type,source_url,provider,external_source_id,title,storage_object_key,sha256,ingest_status) VALUES($1::uuid,$2::source_origin_enum,$3,$4,$5,$6,$7,$8,$9::source_ingest_status_enum)", source_id, body["origin_type"], body.get("source_url"), body["provider"], body.get("external_source_id"), body.get("title"), source_upload["object_key"] if source_upload else None, source_upload["verified_sha256"] if source_upload else None, ingest)
...
async def record_submission(request: Request, owner: str = Depends(require_owner), idempotency_key: str | None = Header(default=None, alias="Idempotency-Key")):
    ...
    async with owner_transaction(owner) as conn:
        ...
        row = await conn.fetchrow("SELECT * FROM submissions WHERE id=$1::uuid", sid); response = {"id":sid,"campaign_id":str(row["campaign_id"]),"post_id":str(row["post_id"]),"submitted_at":_iso(row["submitted_at"]),"submission_reference":row["submission_reference"],"status":str(row["status"]),"created_at":_iso(row["created_at"]),"updated_at":_iso(row["updated_at"])}; validate_contract("SubmissionRecord", response); await _emit_fact_event(conn,event_name="SUBMISSION_RECORDED",entity_type="submission",entity_id=sid,correlation_id=str(uuid4()),payload={"event_name":"SUBMISSION_RECORDED","subject_type":"submission","subject_id":sid,"state":str(row["status"]),"reason_code":None,"evidence_id":None,"amount_usd":None,"related_ids":[str(row["campaign_id"]),str(row["post_id"])]},idempotency_key=key); await _store_response(conn, owner=owner, key=key, method="POST", path="/v1/submissions", body=raw, status=201, response=response); return response
...
async def record_analytics_checkin(request: Request, owner: str = Depends(require_owner), idempotency_key: str | None = Header(default=None, alias="Idempotency-Key")):
    ...
    async with owner_transaction(owner) as conn:
        ...
        if replay is not None: return replay
        if await conn.fetchrow("SELECT id FROM posts WHERE id=$1::uuid", body["post_id"]) is None: raise HonorError(409, "CONFLICT", "Post is not available for analytics.")
        checkin = None
        if body["checkin_id"]:
        ...
        validate_contract("AnalyticsCheckinResult", response); await _emit_fact_event(conn,event_name="ANALYTICS_RECEIVED",entity_type="analytics_observation",entity_id=oid,correlation_id=str(uuid4()),payload={"event_name":"ANALYTICS_RECEIVED","subject_type":"analytics_observation","subject_id":oid,"state":"RECEIVED","reason_code":None,"evidence_id":body.get("evidence_upload_id"),"amount_usd":None,"related_ids":[body["post_id"]]},idempotency_key=key); await _store_response(conn, owner=owner, key=key, method="POST", path="/v1/analytics/check-ins", body=raw, status=201, response=response); return response
...
async def payout_events(request: Request, owner: str = Depends(require_owner), idempotency_key: str | None = Header(default=None, alias="Idempotency-Key")):
    ...
    async with owner_transaction(owner) as conn:
        ...
        if body["event_kind"] == "CREATE":
            ...
            snapshot = campaign["terms_snapshot_id"]
            if loc["post_id"]:
                historical = await conn.fetchval("SELECT c.rule_snapshot_id FROM posts p JOIN clips c ON c.id=p.clip_id WHERE p.id=$1::uuid AND c.campaign_id=$2::uuid", loc["post_id"], loc["campaign_id"])
                if historical is None: raise HonorError(409, "CONFLICT", "Earning post lineage mismatch.")
                snapshot = historical
            duplicate = await conn.fetchrow("SELECT id FROM earnings WHERE campaign_id=$1::uuid AND external_earning_id=$2", loc["campaign_id"], loc["external_earning_id"])
            if duplicate is not None:
                raise HonorError(409, "EARNING_ALREADY_EXISTS", "Earning already exists.")
            eid = str(uuid4())
            try:
                await conn.execute("INSERT INTO earnings(id,campaign_id,rule_snapshot_id,post_id,external_earning_id,amount_usd,state,recognized_at,last_state_at,evidence_snapshot_id,source,idempotency_key) VALUES($1::uuid,$2::uuid,$3::uuid,$4::uuid,$5,$6,'ACCRUED_UNVERIFIED',$7,$7,$8::uuid,$9,$10)", eid, loc["campaign_id"], snapshot, loc["post_id"], loc["external_earning_id"], Decimal(body["amount_usd"]), body["occurred_at"], body["evidence_snapshot_id"], body["source"], key)
            except Exception as exc:
                raise HonorError(409, "EARNING_ALREADY_EXISTS", "Earning already exists.") from exc
        else:
            ...
            except Exception as exc: raise HonorError(409, "INVALID_STATE_TRANSITION", "Invalid earning transition.") from exc
            eid = body["earning_id"]
        result = await conn.fetchrow("SELECT * FROM earnings WHERE id=$1::uuid", eid); response = _earning_record(result); validate_contract("EarningRecord", response)
        event_by_state = {"ACCRUED_UNVERIFIED":"PAYOUT_ACCRUED_UNVERIFIED","APPROVED":"PAYOUT_APPROVED","WITHDRAWABLE":"PAYOUT_WITHDRAWABLE","WITHDRAWN":"PAYOUT_WITHDRAWN"}
        event_name = event_by_state.get(str(result["state"]))
        if event_name:
            await _emit_fact_event(conn,event_name=event_name,entity_type="earning",entity_id=eid,correlation_id=str(uuid4()),payload={"event_name":event_name,"subject_type":"earning","subject_id":eid,"state":str(result["state"]),"reason_code":None,"evidence_id":str(result["evidence_snapshot_id"]) if result["evidence_snapshot_id"] else None,"amount_usd":f"{Decimal(result['amount_usd']):.6f}","related_ids":[str(result["campaign_id"])]},idempotency_key=key)
        await _store_response(conn, owner=owner, key=key, method="POST", path="/v1/payout-events", body=raw, status=201, response=response); return response
...
def build_c02_polli_gateway_handlers(conn):
    async def finance(args):
        earnings = [dict(row) for row in await conn.fetch("SELECT state,amount_usd,recognized_at FROM earnings")]; costs = [dict(row) for row in await conn.fetch("SELECT service,cost_category,estimated_cost_usd,actual_cost_usd,unit,reconciled_at,external_usage_id,incurred_at FROM cost_ledger")]
        cutoff = datetime.fromisoformat(args["to"].replace("Z", "+00:00")) if args.get("to") else _now()
        start = datetime.fromisoformat(args["from"].replace("Z", "+00:00")) if args.get("from") else None
        return polli_envelope("finance_summary", summarize_finance(earnings, costs, as_of=cutoff, period_start=start).to_dict())
    ...
    async def campaign_performance(_args): return polli_envelope("campaign_performance", {"items": []}, truth_label="UNKNOWN")
    ...
    async def account_performance(_args): return polli_envelope("account_performance", {"items": []}, truth_label="UNKNOWN")
